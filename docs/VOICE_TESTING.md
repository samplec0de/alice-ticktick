...
**Слоты:** `task_name` (STRING), `date` (DATETIME), `priority` (STRING), `rec_freq` (STRING), `rec_interval` (NUMBER), `rec_monthday` (NUMBER), `reminder_value` (NUMBER), `reminder_unit` (STRING), `duration_value` (NUMBER), `duration_unit` (STRING), `range_start` (DATETIME), `range_end` (DATETIME)

**Ожидаемые ответы:**
- Без даты: `Готово! Задача "..." создана.`
- С датой: `Готово! Задача "..." создана на ...`
- С повторением: `Готово! Задача "..." создана, каждый понедельник.`
- С напоминанием: `Готово! Задача "..." создана с напоминанием за 30 минут.`
- С повторением и напоминанием: `Готово! Задача "..." создана, каждый день, напоминание за 1 час.`
- С длительностью: `Готово! "..." на ..., HH:MM до HH:MM.`
...
| 8 | «создай задачу полить цветы каждый день» | name=полить цветы, rec_freq=день | Создана с повторением | [ ] |
| 9 | «создай задачу оплатить аренду каждое 15 число» | name=оплатить аренду, rec_monthday=15 | Создана с повторением | [ ] |
| 10 | «создай задачу пробежка каждые 2 дня» | name=пробежка, rec_interval=2, rec_freq=дня | Создана с повторением | [ ] |
| 11 | «создай задачу встреча на завтра с напоминанием за 30 минут» | name=встреча, date=завтра, reminder_value=30, reminder_unit=минут | Создана с датой и напоминанием | [ ] |
| 12 | «создай задачу позвонить врачу с напоминанием за час» | name=позвонить врачу, reminder_unit=час | Создана с напоминанием (за 1 час) | [ ] |
| 13 | «создай задачу зарядка ежедневно с напоминанием за 15 минут» | name=зарядка, rec_freq=ежедневно, reminder_value=15, reminder_unit=минут | Создана с повторением и напоминанием | [ ] |
| 14 | «создай встречу совещание завтра в 10 на 2 часа» | name=совещание, date=завтра в 10, duration_value=2, duration_unit=часа | Готово! «Совещание» на завтра, 10:00 до 12:00. | [ ] |
| 15 | «добавь встречу ланч завтра в 12 на час» | name=ланч, date=завтра в 12, duration_unit=час | Готово! «Ланч» на завтра, 12:00 до 13:00. | [ ] |
| 16 | «создай встречу стендап завтра в 10 на полчаса» | name=стендап, date=завтра в 10, duration_unit=полчаса | Готово! «Стендап» на завтра, 10:00 до 10:30. | [ ] |
| 17 | «создай задачу митинг с 14 до 16» | name=митинг, range_start=14, range_end=16 | Готово! «Митинг» на сегодня, 14:00 до 16:00. | [ ] |
| 18 | «создай встречу на час» | name=?, duration_unit=час, date=пусто | Во сколько начинается? Скажите время. | [ ] |
| 19 | «создай встречу совещание завтра в 10 на 2 часа с напоминанием за 15 минут» | name=совещание, date=завтра в 10, duration_value=2, duration_unit=часа, reminder_value=15, reminder_unit=минут | Готово! «Совещание» на завтра, 10:00 до 12:00, напоминание за 15 минут. | [ ] |
...
- «каждый день» — слово «каждый» может поглощаться в название задачи, если идёт после `.+`
- «с напоминанием за час» — без числа, значение по умолчанию 1 (за 1 час)
...
**Ожидаемые ответы:**
- Есть просроченные: `Просроченных — [N]: ...`
...
**Intent ID:** `edit_task`
**Слоты:** `task_name` (STRING), `new_date` (DATETIME), `new_priority` (STRING), `new_name` (STRING), `new_project` (STRING), `rec_freq` (STRING), `rec_interval` (NUMBER), `rec_monthday` (NUMBER), `reminder_value` (NUMBER), `reminder_unit` (STRING), `remove_recurrence` (STRING), `remove_reminder` (STRING)

**Ожидаемые ответы:**
- Дата/приоритет/название: `Задача "..." обновлена.`
- Перемещение в проект: `Задача "..." перемещена в проект "...".`
- Изменение повторения: `Повторение задачи "..." изменено: каждый понедельник.`
- Удаление повторения: `Повторение задачи "..." убрано.`
- Изменение напоминания: `Напоминание задачи "..." изменено: за 30 минут.`
- Удаление напоминания: `Напоминание задачи "..." убрано.`
- Нет изменений: `Не удалось понять, что изменить. Скажите, например: ...`
...
| # | Фраза | Слоты | Ожидание | Статус |
|---|---|---|---|---|
| 5 | «перемести задачу отчёт в проект Работа» | name=отчёт, new_project=Работа | Задача перемещена | [ ] |
| 6 | «перекинь задачу покупки в список Дом» | name=покупки, new_project=Дом | Задача перемещена | [ ] |

#### Изменение повторения

| # | Фраза | Слоты | Ожидание | Статус |
|---|---|---|---|---|
| 7 | «поменяй повторение задачи зарядка на каждый день» | name=зарядка, rec_freq=день | Повторение изменено | [ ] |
| 8 | «измени повтор задачи уборка на каждую неделю» | name=уборка, rec_freq=неделю | Повторение изменено | [ ] |
| 9 | «поменяй повторение задачи оплата на каждое 15 число» | name=оплата, rec_monthday=15 | Повторение изменено | [ ] |

#### Удаление повторения

| # | Фраза | Слоты | Ожидание | Статус |
|---|---|---|---|---|
| 10 | «убери повторение задачи зарядка» | name=зарядка, remove_recurrence=повторение | Повторение убрано | [ ] |
| 11 | «отмени повтор задачи уборка» | name=уборка, remove_recurrence=повтор | Повторение убрано | [ ] |

#### Изменение напоминания

| # | Фраза | Слоты | Ожидание | Статус |
|---|---|---|---|---|
| 12 | «поменяй напоминание задачи встреча за 30 минут» | name=встреча, reminder_value=30, reminder_unit=минут | Напоминание изменено | [ ] |
| 13 | «измени напоминание задачи отчёт за час» | name=отчёт, reminder_unit=час | Напоминание изменено (за 1 час) | [ ] |
| 14 | «поставь напоминание задачи покупки за 1 день» | name=покупки, reminder_value=1, reminder_unit=день | Напоминание изменено | [ ] |

#### Удаление напоминания

| # | Фраза | Слоты | Ожидание | Статус |
|---|---|---|---|---|
| 15 | «убери напоминание задачи встреча» | name=встреча, remove_reminder=напоминание | Напоминание убрано | [ ] |
| 16 | «отмени напоминание задачи отчёт» | name=отчёт, remove_reminder=напоминание | Напоминание убрано | [ ] |
...
- «убери повторение» vs «убери напоминание» — слова «повторение»/«напоминание» являются ключевыми для определения действия
- «за час» без числа — по умолчанию интерпретируется как «за 1 час»
...
**Ожидаемые ответы:**
- Успех: `«...» добавлен в чеклист задачи «...». Всего пунктов: N.`
...
### 2.15. create_recurring_task — Создание повторяющейся задачи

**Intent ID:** `create_recurring_task`
**Слоты:** `task_name` (STRING), `rec_freq` (STRING), `rec_interval` (NUMBER), `rec_monthday` (NUMBER)

**Особенность:** альтернативный интент для создания повторяющихся задач с формулировкой «напоминай каждый...». Делегирует в `create_task` с заполненными слотами повторения.

**Ожидаемые ответы:**
- `Готово! Задача "..." создана, каждый понедельник.`
- `Готово! Задача "..." создана, каждый день.`
- `Готово! Задача "..." создана, каждое 1 число месяца.`

| # | Фраза | Слоты | Ожидание | Статус |
|---|---|---|---|---|
| 1 | «напоминай каждый понедельник проверить отчёт» | rec_freq=понедельник, name=проверить отчёт | Задача создана с повторением | [ ] |
| 2 | «напоминай каждый день пить воду» | rec_freq=день, name=пить воду | Создана, каждый день | [ ] |
| 3 | «напоминай ежедневно делать зарядку» | rec_freq=ежедневно, name=делать зарядку | Создана, каждый день | [ ] |
| 4 | «напоминай еженедельно проверить почту» | rec_freq=еженедельно, name=проверить почту | Создана, каждую неделю | [ ] |
| 5 | «напоминай ежемесячно оплатить аренду» | rec_freq=ежемесячно, name=оплатить аренду | Создана, каждый месяц | [ ] |
| 6 | «напоминай каждые 2 дня поливать цветы» | rec_interval=2, rec_freq=дня, name=поливать цветы | Создана, каждые 2 дня | [ ] |
| 7 | «напоминай каждое 15 число оплатить счёт» | rec_monthday=15, name=оплатить счёт | Создана, каждое 15 число | [ ] |
| 8 | «повторяй каждую среду уборка» | rec_freq=среду, name=уборка | Задача создана с повторением | [ ] |

**Возможные проблемы голосового распознавания:**
- «напоминай» может распознаться как «напомни» — это другой интент (`add_reminder`)
- «каждые два дня» — число «два» должно распознаться как YANDEX.NUMBER=2
- «каждое пятнадцатое число» — «пятнадцатое» должно распознаться как YANDEX.NUMBER=15
- Порядок слов: рекуррентная часть идёт ДО названия задачи (особенность грамматики)

---

### 2.16. add_reminder — Добавление напоминания к существующей задаче

**Intent ID:** `add_reminder`
**Слоты:** `task_name` (STRING), `reminder_value` (NUMBER), `reminder_unit` (STRING)

**Особенность:** добавляет напоминание к уже существующей задаче (не создаёт новую). Если напоминание уже есть — не дублирует.

**Ожидаемые ответы:**
- Успех: `Напоминание за 30 минут добавлено к задаче "...".`
- Задача не найдена: `Задача "..." не найдена. Попробуйте сказать точнее.`
- Не указано время: `За сколько напомнить? Скажите, например, «за 30 минут» или «за час».`

| # | Фраза | Слоты | Ожидание | Статус |
|---|---|---|---|---|
| 1 | «напомни о задаче встреча за 30 минут» | name=встреча, reminder_value=30, reminder_unit=минут | Напоминание добавлено | [ ] |
| 2 | «напомни про задачу отчёт за час» | name=отчёт, reminder_unit=час | Напоминание добавлено (за 1 час) | [ ] |
| 3 | «напомни о задаче покупки за 1 день» | name=покупки, reminder_value=1, reminder_unit=день | Напоминание добавлено | [ ] |
| 4 | «напомни про задачу зарядка за 15 минут» | name=зарядка, reminder_value=15, reminder_unit=минут | Напоминание добавлено | [ ] |
| 5 | «поставь напоминание о задаче оплата за 2 часа» | name=оплата, reminder_value=2, reminder_unit=часа | Напоминание добавлено | [ ] |
| 6 | «напомни о задаче врач за день» | name=врач, reminder_unit=день | Напоминание добавлено (за 1 день) | [ ] |

**Возможные проблемы голосового распознавания:**
- «напомни» (add_reminder) vs «напоминай» (create_recurring_task) — похожие слова, разные интенты
- «за час» без числа — значение по умолчанию 1 (за 1 час)
- «за день» без числа — значение по умолчанию 1 (за 1 день)
- Предлоги «о»/«про»/«на»/«для» перед «задаче» — все поддерживаются грамматикой
- «поставь напоминание» — альтернативная формулировка, может конфликтовать с «поставь задачу» (create_task)

---

### 2.17. помощь (YANDEX.HELP / YANDEX.WHAT_CAN_YOU_DO)
...
### 2.18. до свидания (YANDEX.GOODBYE)
...
| «напомни о задаче X за час» | `add_reminder` | `create_recurring_task`? | «напомни» vs «напоминай» |
| «поставь напоминание о задаче X за час» | `add_reminder` | `create_task` | «поставь» + «напоминание» |
...
| # | Фраза | Ожидание | Статус |
|---|---|---|---|
| 1 | «расскажи анекдот» | Fallback: «Команда не распознана. Скажите «помощь»...» | [ ] |
...
| 16 | создай задачу полить цветы каждый день | create_task | [ ] | Повторение через create_task |
| 17 | создай задачу встреча с напоминанием за 30 минут | create_task | [ ] | Напоминание через create_task |
| 18 | создай задачу встреча с напоминанием за час | create_task | [ ] | Напоминание без числа |
| 19 | напоминай каждый понедельник проверить отчёт | create_recurring_task | [ ] | Отдельный интент |
| 20 | напоминай ежедневно делать зарядку | create_recurring_task | [ ] | Ежедневное повторение |
| 21 | напомни о задаче встреча за 30 минут | add_reminder | [ ] | Напоминание к существующей задаче |
| 22 | напомни про задачу отчёт за час | add_reminder | [ ] | Без числа (по умолчанию 1) |
| 23 | поменяй повторение задачи зарядка на каждый день | edit_task | [ ] | Изменение повторения |
| 24 | убери повторение задачи зарядка | edit_task | [ ] | Удаление повторения |
| 25 | поменяй напоминание задачи встреча за 30 минут | edit_task | [ ] | Изменение напоминания |
| 26 | убери напоминание задачи встреча | edit_task | [ ] | Удаление напоминания |
...
- [ ] «создай задачу полить цветы каждый день» — создана с повторением
- [ ] «создай задачу оплатить аренду каждое 15 число» — повторение по дате месяца
- [ ] «создай задачу пробежка каждые 2 дня» — повторение с интервалом
- [ ] «напоминай каждый понедельник проверить отчёт» — create_recurring_task
- [ ] «напоминай ежедневно делать зарядку» — ежедневное повторение
- [ ] «напоминай каждые 2 дня поливать цветы» — повторение с интервалом
- [ ] «напоминай каждое 15 число оплатить счёт» — повторение по дате месяца
- [ ] «поменяй повторение задачи зарядка на каждый день» — изменение повторения
- [ ] «убери повторение задачи зарядка» — удаление повторения

### Напоминания

- [ ] «создай задачу встреча на завтра с напоминанием за 30 минут» — создание с напоминанием
- [ ] «создай задачу позвонить врачу с напоминанием за час» — без числа (по умолчанию 1)
- [ ] «напомни о задаче встреча за 30 минут» — add_reminder к существующей задаче
- [ ] «напомни про задачу отчёт за час» — add_reminder без числа
- [ ] «поставь напоминание о задаче оплата за 2 часа» — альтернативная формулировка
- [ ] «поменяй напоминание задачи встреча за 30 минут» — изменение напоминания
- [ ] «измени напоминание задачи отчёт за час» — изменение без числа
- [ ] «убери напоминание задачи встреча» — удаление напоминания

### Комбинации (повторение + напоминание)

- [ ] «создай задачу зарядка ежедневно с напоминанием за 15 минут» — повторение + напоминание
...
- [ ] `add_reminder` «напомни» — не путается с `create_recurring_task` «напоминай»
- [ ] `add_reminder` «поставь напоминание» — не перехватывается `create_task` «поставь задачу»
